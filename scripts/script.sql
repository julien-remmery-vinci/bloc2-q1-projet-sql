...
BEGIN
    ...
    END IF;
    IF NEW.etat = 'annulée' AND offre.etat = 'annulée' THEN
...
DECLARE
    ...
    sortie RECORD;
BEGIN
    FOR offre IN SELECT * FROM projet.offres_de_stages os WHERE os.identifiant_entreprise = identifiantEntreprise ORDER BY code_offre_stage
        LOOP
...
SELECT *
FROM projet.voirSesOffres('UCL') AS (code_offre_stage VARCHAR(20), description VARCHAR(100), semestre VARCHAR(2),
...
BEGIN
    ...
    UPDATE projet.candidatures c SET etat = 'refusée' WHERE c.id_offre_stage = offre.id_offre_stage;
end;
$$ LANGUAGE plpgsql;
--PARTIE ETUDIANT
...
SELECT *
FROM projet.afficherOffresStage('Q2') AS (code_offre VARCHAR(20), nom_entreprise VARCHAR(50),
